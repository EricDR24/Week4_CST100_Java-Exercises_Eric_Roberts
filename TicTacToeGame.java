/** Program:  8.9 HangmanGame
  * File:     TicTacToeGame.java 
  * Summary:  Chapter 8, Exercise 9, create a tic tac toe game
  * Author:   Eric Roberts
  * Date:     July 17, 2016
**/
	...
	public static void main(String[] args) {
		...
		//repeat while result is continue
			do {
					...
					//mark cell with player's token
					placeToken(board, cell, tokens[0]);
			
					//game status
					result = gameStatus(board, tokens[0]);

					//status is continue make next player take turn
					if (result == 2) {
						swap(tokens);
					}
